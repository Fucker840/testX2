...
def banner():
	...
	sol().print(cik2)
	ban='''
•   AUTHOR  : RAFI-XD WHATSAPP : +8801982876336  •
•   _______         _______ 
  (  ____ |\     /(  ____ )
  | (    \| )   ( | (    )|
  | (_____| (___) | (____)|
  ...
  /\____) | )   ( | ) \ \__
  \_______|/     \|/   \__/ VERSION 4.0.4  •
•                                               •
•   GITHUB : GO AND FUCK YOURSELF •'''
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='RAFI-XD CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='RAFI-XD OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]RAFI XD OK[/bold green]'))
					ok+=1
					break
